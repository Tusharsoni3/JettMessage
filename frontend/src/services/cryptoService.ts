...
export const cryptoService = {
   ...
   */
  async deriveAndSplitPassword(password: string, email: string): Promise<SplitKeys> {
    const encoder = new TextEncoder();
   ...
   */
  async generateMLKEMKeyPair(): Promise<{ publicKey: string; privateKey: string }> {
   ...
   */
  async encryptPrivateKey(privateKeyHex: string, keyB: ArrayBuffer): Promise<EncryptedBundle> {
    const encoder = new TextEncoder();
   ...
   */
  async decryptPrivateKey(bundle: EncryptedBundle, keyB: ArrayBuffer): Promise<string> {
