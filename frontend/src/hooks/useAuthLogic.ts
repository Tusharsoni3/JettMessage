...
export const useAuthLogic = () => {
  ...
  const handleSignup = async (
    ...
    try {
      ...
      const { keyA, keyB } = await cryptoService.deriveAndSplitPassword(
        password,
        email,
      );
      ...
      const encryptedBundle = await cryptoService.encryptPrivateKey(
        keyPair.privateKey,
        keyB,
      );
      ...
      setError(
        err instanceof Error
          ? err.message
          : "An error occurred during secure signup.",
      );
    } finally {
      setLoading(false);
    }
